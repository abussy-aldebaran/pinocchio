...
namespace pinocchio
{
      ...
      TangentVectorType2>>
  {
      ...
      const Eigen::MatrixBase<TangentVectorType2> & a)
    {
      ...
      motionSet::motionAction(ov, J_cols, psid_cols);  // This ov here is v(p(i)), psi_dot calcs
      motionSet::motionAction(oa, J_cols, psidd_cols); // This oa here is a(p(i)) , psi_dotdot calcs
      ...
      Tensor4>>
  {
      ...
      const Tensor3 & dtau_dadq)
    {
      ...
      for (int p = 0; p < model.nvs[i]; p++)
      {
        ...
        const MotionRef<typename Data::Matrix6x::ColXpr> psid_dm =
          data.psid.col(ip); // psi_dot for p DOF
        const MotionRef<typename Data::Matrix6x::ColXpr> psidd_dm =
          data.psidd.col(ip); // psi_ddot for p DOF
        const MotionRef<typename Data::Matrix6x::ColXpr> phid_dm =
          data.dJ.col(ip); // phi_dot for p DOF
        ...
        r3.noalias() = oBicpsidot - S_iA.transpose() * oBcrb
        ...
        while (j > 0)
        {
          ...
          for (int q = 0; q < model.nvs[j]; q++)
          {
            ...
            const MotionRef<typename Data::Matrix6x::ColXpr> psid_dm =
              data.psid.col(jq); // psi_dot{j}(:,q)
            const MotionRef<typename Data::Matrix6x::ColXpr> psidd_dm =
              data.psidd.col(jq); // psi_ddot{j}(:,q)
            const MotionRef<typename Data::Matrix6x::ColXpr> phid_dm =
              data.dJ.col(jq); // phi_dot{j}(:,q)
            ...
            while (k > 0)
            {
              ...
              for (int r = 0; r < model.nvs[k]; r++)
              {
                ...
                const MotionRef<typename Data::Matrix6x::ColXpr> psid_dm =
                  data.psid.col(kr); // psi_dot{k}(:,r)
                const MotionRef<typename Data::Matrix6x::ColXpr> psidd_dm =
                  data.psidd.col(kr); // psi_ddot{k}(:,r)
                const MotionRef<typename Data::Matrix6x::ColXpr> phid_dm =
                  data.dJ.col(kr); // phi_dot{k}(:,r)
    ...
    template<typename ForceDerived, typename M6>
    static void
    ForceCrossMatrix(const ForceDense<ForceDerived> & f, const Eigen::MatrixBase<M6> & mout)
    {
      M6 & mout_ = PINOCCHIO_EIGEN_CONST_CAST(M6, mout);
      mout_.template block<3, 3>(ForceDerived::LINEAR, ForceDerived::LINEAR).setZero();
      mout_.template block<3, 3>(ForceDerived::LINEAR, ForceDerived::ANGULAR) =
        mout_.template block<3, 3>(ForceDerived::ANGULAR, ForceDerived::LINEAR) = skew(-f.linear());
    ...
    const Tensor4 & dtau_dadq)
  {
    ...
    typedef ComputeRNEASecondOrderDerivativesBackwardStep<
      Scalar, Options, JointCollectionTpl, Tensor1, Tensor2, Tensor3, Tensor4>
      Pass2;
    for (JointIndex i = (JointIndex)(model.njoints - 1); i > 0; --i)
    {
      Pass2::run(
        model.joints[i],
        typename Pass2::ArgsType(
          model, data, const_cast<Tensor1 &>(d2tau_dqdq), const_cast<Tensor2 &>(d2tau_dvdv),
          const_cast<Tensor3 &>(dtau_dqdv), const_cast<Tensor4 &>(dtau_dadq)));
    }
  }
